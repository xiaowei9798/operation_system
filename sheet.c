...
void sheet_updown(struct SHEET *sht, int height)
{
	...
	if (old > height) {	/* 以前よりも低くなる */
		...
		sheet_refreshsub(ctl,sht->vx0,sht->vy0,sht->vx0+sht->bxsize,sht->vy0+sht->bysize,0,old-1); /* 按照新图层的信息绘制画面 */
		...
		sheet_refreshsub(ctl,sht->vx0,sht->vy0,sht->vx0+sht->bxsize,sht->vy0+sht->bysize,height,height); /* 新しい下じきの情報に沿って画面を描き直す */
	}
	return;
}
...
void sheet_slide(struct SHEET *sht, int vx0, int vy0)
{
	...
	if (sht->height >= 0) { /* もしも表示中なら */
		...
		sheet_refreshsub(sht->ctl,old_vx0,old_vy0,old_vx0+sht->bxsize,old_vy0+sht->bysize,0,sht->height-1); /*图层没移动前刷新一次 */
		sheet_refreshsub(sht->ctl,vx0,vy0,vx0+sht->bxsize,vy0+sht->bysize,sht->height,sht->height);//图层移动后，在刷新一次
	}
	return;
}

void sheet_free(struct SHEET *sht)//将已使用的图层的内存释放出来
{
	if (sht->height >= 0) {
